var custom_dimensions = {
  ...
  init: function () {
    ...
    jQuery("#exactmetrics_add_row").click(this.add_row);

    jQuery(document).on("click", "[id^=exactmetrics_remove_]", this.remove_row);
    jQuery(document).on('change', "select[name^='exactmetrics_settings[custom_dimensions]']", this.row_change_options);
    jQuery(document).on('change', "input[name^='exactmetrics_settings[custom_dimensions]']", this.row_change_id);
    this.row_init_options();
  },

  toggle_add_button: function () {
    if (tmp_total < limit) {
      jQuery("#exactmetrics_add_row").show();
    } else {
      jQuery("#exactmetrics_add_row").hide();
    }
  },
  ...
  add_row: function () {
    if (tmp_total < limit) {
      ...
      jQuery("tbody", "#exactmetrics-custom_dimensions").append('<tr id="exactmetrics-' + total + '"><td><select name="exactmetrics_settings[custom_dimensions][' + total + '][type]">' + options_to_add + '</select></td><td align="left"><input type="text" name="exactmetrics_settings[custom_dimensions][' + total + '][id]" value="' + total + '" style="width: 50px;" /></td><td><a href="#" id="exactmetrics_remove_' + total + '" class="exactmetrics-settings-click-excluded">' + translate_delete + '</a></td></tr>');

      custom_dimensions.set_limit_value(tmp_total);

      var current_select = jQuery('#exactmetrics-' + total + ' select');
  ...
  remove_row: function () {
    ...
    var current_select_box = jQuery(this).closest('tr').find("select[name^='exactmetrics_settings[custom_dimensions]']");
    custom_dimensions.row_show_options(current_select_box);

    var old_id = this.id;
    var new_id = old_id.replace('exactmetrics_remove_', '');
    var disabled = !!jQuery("#exactmetrics-" + new_id + " select:disabled")[0];

    jQuery("#exactmetrics-" + new_id).remove();
  ...
  row_init_options: function () {
    var select_boxes = jQuery("select[name^='exactmetrics_settings[custom_dimensions]']");
    select_boxes.each(
  ...
  row_change_id: function () {
    ...
    jQuery("input[name^='exactmetrics_settings[custom_dimensions]']").each(function () {
    ...
    if (total_matches >= 2) {
      ...
      jQuery("#exactmetrics-custom_dimensions").prepend('<tr id="custom-dimension-error"><td colspan="2"><strong><font color="red">' + jQuery("#string_error_custom_dimensions").val() + '</font></strong></td></tr>');
    }
  },
  ...
  set_limit_value: function (new_value) {
    jQuery('#exactmetrics_limit').html(new_value);
  },


  get_other_selects: function (exclude_select) {
    var select_boxes = jQuery("select[name^='exactmetrics_settings[custom_dimensions]']").not(exclude_select);
    return select_boxes;
  },
