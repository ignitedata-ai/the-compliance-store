...
class ExactMetrics_Frontend_Custom_Dimensions {
	...
	private function get_custom_dimension_value( $type ) {
		...
		switch ( $type ) {
			...
			case 'publish_day_of_week':
				$value = $this->get_publish_day_of_week_dimension();
				break;
			case 'modified_at':
				$value = $this->get_modified_at_dimension();
				break;
			case 'focus_keyword':
			...
			case 'aioseo_focus_keyphrase':
				...
				break;
			case 'rankmath_seo_score':
				$value = $this->rankmath_get_seo_score();
				break;
			case 'rankmath_focus_keyword':
				$value = $this->rankmath_get_focus_keyword();
				break;
			case 'seopress_seo_score':
				$value = $this->seopress_get_seo_score();
				break;
			case 'seopress_keywords':
				$value = $this->seopress_get_keywords();
				break;
			case 'inactive':
			default:
				// don't do anything
	...
	protected function get_published_at_dimension() {
		...
		return apply_filters( 'exactmetrics_dimensions_tracking_published_at', $date );
	}

	/**
	 * Returns the day of the week when the post was published.
	 */
	protected function get_publish_day_of_week_dimension() {
		$date = '';
		if ( is_singular() ) {
			$date = get_the_date( 'l' );
		}
	...
	/**
	 * Get single post/page modified date.
	 *
	 * @return string
	 */
	protected function get_modified_at_dimension() {
		$date = '';
		if ( is_singular() ) {
			$date = get_the_modified_date( 'c' );
		}

		return apply_filters( 'exactmetrics_dimensions_tracking_modified_at', $date );
	}
	...
	protected function aioseo_get_keyphrase() {
		...
		if ( function_exists( 'exactmetrics_is_aioseo_active' ) && exactmetrics_is_aioseo_active() && is_singular() ) {
			...
			if ( is_object( $result ) && property_exists( $result, 'keyphrases' ) ) {
                ...
                $result = is_string( $result->keyphrases ) ? json_decode( $result->keyphrases ) : $result;
	...
	/**
	 * Get RankMath SeoScore.
	 *
	 * @return string
	 *
	 */
	protected function rankmath_get_seo_score() {

		if ( class_exists( '\RankMath\Frontend_SEO_Score' ) && is_singular() ) {
			$rankmath = new \RankMath\Frontend_SEO_Score();
			$score    = $rankmath->get_score( get_the_ID() );
	...
	/**
	 * Get RankMath Focus Keyword.
	 *
	 * @return string
	 *
	 */
	protected function rankmath_get_focus_keyword() {
	...
	/**
	 * Get SeoPress SeoScore.
	 *
	 * @return string
	 *
	 */
	protected function seopress_get_seo_score() {

		if ( function_exists( 'seopress_get_service' ) && is_singular() ) {
			$post = get_post();
			...
			if ( in_array('medium', $impact) || in_array('high', $impact) ) {
				return esc_html__('Should be improved', 'exactmetrics-premium');
			} else {
				return esc_html__('Good', 'exactmetrics-premium');
			}
	...
	/**
	 * Get SeoPress keywords.
	 * Since we are forced to get them all in one string we will return a list of kewords separated by a comma.
	 *
	 * @return string
	 *
	 */
	protected function seopress_get_keywords() {
