...
<div class="wpfl-4 wpforo-section">
    ...
    <?php foreach( $posts as $key => $post ) : ?>
        ...
        <div id="post-<?php echo wpforo_bigintval( $post['postid'] ) ?>" data-postid="<?php echo wpforo_bigintval( $post['postid'] ) ?>"
             data-userid="<?php echo wpforo_bigintval( $member['userid'] ) ?>"
             data-mention="<?php echo esc_attr( ( wpforo_setting( 'profiles', 'mention_nicknames' ) ? $member['user_nicename'] : '' ) ) ?>"
             data-isowner="<?php echo esc_attr( (int) (bool) wpforo_is_owner( $member['userid'] ) ) ?>"
                ...
                <div class="wpf-left">
                    ...
                    <div class="wpf-author-data">
                        ...
                        <?php if( empty( $wpf_is_anon ) ): ?>
                            <div class="wpf-author-posts"><?php wpforo_phrase( 'Posts' ) ?>: <?php echo intval( $member['posts'] ) ?></div><?php endif; ?>
                    </div>
                    ...
                    <div class="wpf-content-head">
                        ...
                        <div class="wpf-content-head-bottom">
                            <div class="wpf-author">
                                ...
                                <?php endif; ?>
                                <?php if( empty( $wpf_is_anon ) ): ?>
                                    <div class="wpf-author-joined"><i class="fas fa-calendar-alt"></i> <?php wpforo_phrase( 'Joined' ) ?>: <?php wpforo_date( $member['user_registered'] ); ?>
                                    </div><?php endif; ?>
                            </div>
            ...
            <?php if( $topic['posts'] > 1 ): ?>
                ...
                <div id="wpf-replies-sep" class="wpf-replies-sep">
                    <div class="wpf-replies-title"><?php $replies_count = '<span class="wpf-replies-count">' . ( $topic['posts'] - 1 ) . '</span>' ?>
                        <i class="far fa-comment"></i> <?php ( (int) $topic['posts'] > 2 ) ? printf( wpforo_phrase( '%s Replies', false ), $replies_count ) : printf(
                                wpforo_phrase( '%s Reply', false ),
                                $replies_count
                        ); ?>
                    </div>
                </div>
            ...
            <?php if( $reply_count && wpforo_setting( 'topics', 'layout_threaded_nesting_level' ) ): ?>
                <div class="wpf-post-replies-bar" id="wpf-ttgg-<?php echo intval( $post['postid'] ); ?>">
                    <div class="wpf-post-replies-info">
                        ...
                        <span class="wpf-post-replies-title"><?php ( (int) $reply_count > 1 ) ? printf( wpforo_phrase( '%s Replies', false ), wpforo_print_number( $reply_count ) ) : printf(
                                    wpforo_phrase( '%s Reply', false ),
                                    wpforo_print_number( $reply_count )
                            ); ?></span>
                    </div>
