...
if ( ! class_exists( 'WPSC_EP_Logger' ) ) :

	final class WPSC_EP_Logger {
		...
		public static function apply_schema() {
			...
			self::$schema = apply_filters( 'wpsc_ep_logger_schema', $schema );

			// Prevent modify.
			$prevent_modify       = array( 'id' );
			self::$prevent_modify = apply_filters( 'wpsc_ep_logger_prevent_modify', $prevent_modify );
		}
		...
		public function __construct( $id = 0 ) {
			...
			if ( $id > 0 ) {
				...
				$tag = $wpdb->get_row( "SELECT * FROM {$wpdb->prefix}psmsc_ep_logger WHERE id = " . $id, ARRAY_A );
				if ( ! is_array( $tag ) ) {
					return;
				}
		 ...
		 *
		 * @param WPSC_EP_Logger $tag - tag object.
		 * @return boolean
		 */
		...
		public static function find( $filter = array(), $is_object = true ) {
			...
			$sql = 'SELECT * FROM ' . $wpdb->prefix . 'psmsc_ep_logger ';
			...
			// total results.
			$sql = 'SELECT count(id) FROM ' . $wpdb->prefix . 'psmsc_ep_logger ';
