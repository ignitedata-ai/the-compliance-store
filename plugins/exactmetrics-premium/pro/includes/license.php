...
final class ExactMetrics_License {
	...
	public function get_site_license_error() {
		if ( $this->site_license_expired() ) {
			...
			return sprintf( esc_html__( 'Your license key for ExactMetrics has expired. %1$sPlease click here to renew your license key.%2$s', 'exactmetrics-premium' ), '<a href="' . exactmetrics_get_url( 'license-error', 'expired-license', 'https://www.exactmetrics.com/login/' ) . '" target="_blank" rel="noopener noreferrer" referrer="no-referrer">', '</a>' );
		} else if ( $this->site_license_disabled() ) {
			return esc_html__( 'Your license key for ExactMetrics has been disabled. Please use a different key.', 'exactmetrics-premium' );
		} else if ( $this->site_license_invalid() ) {
			return esc_html__( 'Your license key for ExactMetrics is invalid. The key no longer exists or the user associated with the key has been deleted. Please use a different key.', 'exactmetrics-premium' );
		}
