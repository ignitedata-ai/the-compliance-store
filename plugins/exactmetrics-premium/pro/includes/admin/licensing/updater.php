...
class ExactMetrics_Updater {
	...
	public function maybe_show_license_expired_message( $plugin_data, $response ) {
		...
		if ( empty( $response->package ) ) {
			...
			echo '<br />' . sprintf( __( 'In order to enable updates, you need to have a valid license key on the %1$ssettings page%2$s. If your license key is expired or you need a new key, then %3$sclick here to purchase ExactMetrics Pro%2$s.', 'exactmetrics-premium' ), '<a href="' . esc_url( $settings_url ) . '">', '</a>', '<a href="' . exactmetrics_get_url( 'Plugin update', $this->plugin_name ) . '">' ); // phpcs:ignore
		}
	}
	...
	public function update_plugins_filter( $value ) {
		...
		if ( ! $this->update ) {
			$this->update = exactmetrics_perform_remote_request( 'get-plugin-update', array( 'tgm-updater-plugin' => $this->plugin_slug ) );
	...
	public function set_plugins_api( $default_api ) {
		...
		if ( ! $this->info ) {
			$this->info = exactmetrics_perform_remote_request( 'get-plugin-info', array( 'tgm-updater-plugin' => $this->plugin_slug ) );
