...
final class ExactMetrics_Report_PopularPosts extends ExactMetrics_Report {

	public $title;
	public $class = 'ExactMetrics_Report_PopularPosts';
	...
	public function __construct() {
		$this->title = __( 'Popular Posts', 'exactmetrics-premium' );
		parent::__construct();
	}
	...
	public function requirements( $error = false, $args = array(), $name = '' ) {
		...
		if ( ! class_exists( 'ExactMetrics_Dimensions' ) ) {
			...
			$text = __( 'Please %s the ExactMetrics Dimensions addon to load popular posts.', 'exactmetrics-premium' );
		...
		if ( empty( $dimension_id ) || 0 === $dimension_id ) {
			...
			return __( 'Please enable the "Post type" custom dimension in order to load Popular Posts data correctly.', 'exactmetrics-premium' );
		}
	...
	public function add_dimensions_settings_link( $data ) {
		if ( current_user_can( 'exactmetrics_save_settings' ) ) {
			$data['data']['footer'] = sprintf( '<a href="%s">%s</a>', admin_url( 'admin.php?page=exactmetrics_settings#/conversions' ), esc_html__( 'Update dimensions settings', 'exactmetrics-premium' ) );
		}
