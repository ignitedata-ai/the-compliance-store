...
//		add_action( 'admin_footer', array( $this, 'load_notice' ) );
	...
	public function widget_scripts() {
		...
		if ( isset( $screen->id ) && 'dashboard' === $screen->id ) {
			...
			$text_domain = exactmetrics_is_pro_version() ? 'exactmetrics-premium' : 'exactmetrics-premium';
		...
		if ( ! ExactMetrics()->license->site_license_expired() && ! ExactMetrics()->license->network_license_expired() ) {
			...
			<div class="mi-dw-not-authed">
				...
				<?php if ( current_user_can( 'exactmetrics_save_settings' ) ) { ?>
					...
					<a class="mi-dw-btn-large exactmetrics-setup-wizard-link"><?php esc_html_e( 'Setup Website Analytics', 'exactmetrics-premium' ); ?></a>
					   <p><?php esc_html_e( 'Note: You will be transfered to ExactMetrics.com to complete the setup wizard.', 'exactmetrics-premium' ); ?></p>
				<?php } else { ?>
			...
			<div class="mi-dw-license-expired">
				...
				echo sprintf( esc_html__( 'Your ExactMetrics %1$s License has expired.', 'exactmetrics-premium' ), ucfirst( $type ) ); // phpcs:ignore
				?></h2>
				<?php if ( current_user_can( 'exactmetrics_save_settings' ) ) { ?>
