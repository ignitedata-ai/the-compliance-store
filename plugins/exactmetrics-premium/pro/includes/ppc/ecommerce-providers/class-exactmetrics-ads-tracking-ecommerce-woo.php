...
/**
 * ExactMetrics PPC Tracking - WooCommerce tracking class.
 *
 * @access public
 * @since 9.7.0
 *
...
class ExactMetrics_Ads_Tracking_Ecommerce_Woo extends ExactMetrics_Ads_Tracking_Ecommerce_Tracking {
	/**
	 * WooCommerce order
	 *
	 * @var $order WC_Order
	...
	public function is_active() {
		return class_exists( 'WooCommerce' ) && apply_filters( 'exactmetrics_ppc_tracking_track_conversion_woocommerce', true );
	}
	...
	public function already_tracked_frontend( $provider_id ) {
		...
		$tracked = $this->order->get_meta( "_exactmetrics_ppc_tracking_conversion_tracked_{$provider_id}" );
	...
	public function mark_order_tracked_frontend( $provider_id ) {
		...
		$this->order->add_meta_data( "_exactmetrics_ppc_tracking_conversion_tracked_{$provider_id}", 'yes', true );
