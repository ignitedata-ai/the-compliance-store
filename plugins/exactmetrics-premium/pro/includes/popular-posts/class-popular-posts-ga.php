...
/**
 * Class ExactMetrics_Popular_Posts_GA
 */
class ExactMetrics_Popular_Posts_GA {
	 ...
	 *
	 * @var ExactMetrics_Popular_Posts_GA
	 */
	 ...
	 *
	 * @return ExactMetrics_Popular_Posts_GA
	 */
	...
	/**
	 * ExactMetrics_Popular_Posts_GA constructor.
	 */
	public function __construct() {

		add_action( 'exactmetrics_popular_posts_grab_ga_data', array( $this, 'try_popular_posts_processing' ) );
	}
	...
	public function save_popular_posts_order( $posts ) {
		...
		$posts      = apply_filters( 'exactmetrics_popular_posts_ga_processed_before_cache', $posts );
		$expiration = strtotime( ' Tomorrow 1am ' ) - ( get_option( 'gmt_offset', 0 ) * HOUR_IN_SECONDS ) - time();
		set_transient( 'exactmetrics_popular_posts_ga_data', $posts, $expiration );
	...
	public function get_popular_posts_from_cache() {
		...
		return get_transient( 'exactmetrics_popular_posts_ga_data' );
	...
	public function get_popular_posts_report() {
		...
		if ( is_null( ExactMetrics()->reporting ) ) {
		...
		if ( ! $site_auth && ! $ms_auth ) {
			return array(
				'error'   => true,
				'message' => __( 'You must authenticate with ExactMetrics before you can view reports.', 'exactmetrics-premium' ),
			);
		}
		...
		/**
		 * @var $report ExactMetrics_Report_PopularPosts The popular posts report object.
		 */
		...
		return array(
			'error'   => true,
			'message' => __( 'We encountered an error when fetching the report data.', 'exactmetrics-premium' ),
		);
	}
	...
	public function dispatch_report_fetching() {
		...
		if ( false === wp_next_scheduled( 'exactmetrics_popular_posts_grab_ga_data' ) ) {
			wp_schedule_single_event( time(), 'exactmetrics_popular_posts_grab_ga_data' );
		}

	}
}

new ExactMetrics_Popular_Posts_GA();
