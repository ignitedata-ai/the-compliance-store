...
/**
 * Class ExactMetrics_Popular_Posts_Products
 */
class ExactMetrics_Popular_Posts_Products extends ExactMetrics_Popular_Posts {
	/**
	 ...
	 * @var string
	 */
	protected $shortcode_key = 'exactmetrics_popular_posts_products';
	...
	public function register_widget() {
		register_widget( 'ExactMetrics_Popular_Posts_Products_Sidebar' );
	}
	...
	public function requirements() {
		...
		if ( ! ExactMetrics()->license->license_can( 'pro' ) ) {
			return false;
		}
	...
	public function get_rendered_html( $atts ) {
		...
		if ( $show_title ) {
			$html .= '<h2 class="exactmetrics-products-popular-posts-title">' . wp_kses_post( $title_text ) . '</h2>';
		}

		$html .= '<ul class="exactmetrics-products-popular-posts-list">';
		...
		foreach ( $posts as $post ) {
			...
			if ( $show_image && ! empty( $post['image'] ) ) {
				$html .= '<div class="exactmetrics-products-popular-posts-image">';
				$html .= '<img src="' . esc_url($post['image']) . '" srcset=" ' . esc_attr($post['srcset']) . ' " alt="' . esc_attr( $post['title'] ) . '" />';
				$html .= '</div>';
			}
			
			$html .= '<div class="exactmetrics-products-popular-posts-text">';
			$html .= '<span class="exactmetrics-products-popular-posts-title" ' . esc_attr($this->get_element_style( $theme, 'title', $atts )) . '>' . esc_html($post['title']) . '</span>';
			if ( $show_price ) {
				$html .= '<span class="exactmetrics-products-popular-posts-price" ' . esc_attr($this->get_element_style( $theme, 'price', $atts )) . '>' . wp_kses_post($post['price']) . '</span>';
			}
			if ( $show_rating ) {
				$rating_color = ! empty( $atts['rating_color'] ) ? $atts['rating_color'] : $theme_styles['styles']['rating']['color'];
				$html         .= '<span class="exactmetrics-products-popular-posts-rating">' . wp_kses_post( $this->get_rating_stars( $rating_color, $post['rating'] ) ) . '</span>';
			}
			$html .= '</div>';// exactmetrics-products-popular-posts-text.
			$html .= '</a>';
			$html .= '</li>';
		}
	...
	public function get_rating_star_svg( $color, $full = false ) {
		...
		return apply_filters( 'exactmetrics_popular_posts_product_rating_star', $star );
	}
	...
	public function get_posts() {
		...
		return apply_filters( 'exactmetrics_popular_posts_products', $posts );
	...
	public function build_inline_styles() {
		...
		foreach ( $themes as $theme_key => $theme_styles ) {
			...
			if ( ! empty( $theme_styles['background'] ) ) {
				$styles .= '.exactmetrics-popular-posts-styled.exactmetrics-products-popular-posts.exactmetrics-products-popular-posts-' . $theme_key . ' .exactmetrics-products-popular-posts-list li {';
			...
			if ( ! empty( $theme_styles['title'] ) ) {
				$styles .= '.exactmetrics-popular-posts-styled.exactmetrics-products-popular-posts.exactmetrics-products-popular-posts-' . $theme_key . ' .exactmetrics-products-popular-posts-list li .exactmetrics-products-popular-posts-title {';
			...
			if ( ! empty( $theme_styles['price'] ) ) {
				$styles .= '.exactmetrics-popular-posts-styled.exactmetrics-products-popular-posts.exactmetrics-products-popular-posts-' . $theme_key . ' .exactmetrics-products-popular-posts-list li .exactmetrics-products-popular-posts-price {';
	...
	public function remove_widget_from_legacy_widgets( $widgets ) {
		// $widgets[] = 'exactmetrics-popular-posts-products';
 ...
 *
 * @return ExactMetrics_Popular_Posts_Products Instance of the current class.
 */
function ExactMetrics_Popular_Posts_Products() {
	return ExactMetrics_Popular_Posts_Products::get_instance();
}

ExactMetrics_Popular_Posts_Products();
