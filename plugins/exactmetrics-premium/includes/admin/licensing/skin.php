	...
	function error( $errors ) {
		...
		if ( ! empty( $errors ) ) {
			...
			$error_message = sprintf(
				esc_html__( 'There was an error installing the addon. Please try again. If you are still having issues, please %1$scontact our support%2$s team.', 'exactmetrics-premium' ),
			...
			if ( is_wp_error( $errors ) ) {
				...
				if ( ! empty( $message ) ) {
					...
					$error_message = sprintf(
						esc_html__( 'There was an error installing the addon, %1$s. Please try again. If you are still having issues, please %2$scontact our support%3$s team. ', 'exactmetrics-premium' ),
						esc_html( $message ),
						'<a target="_blank" href="' . exactmetrics_get_url( 'notice', 'error-installing-addons', 'https://www.exactmetrics.com/my-account/support/' ) . '">',
						'</a>'
					);
