...
if (!class_exists('Faulh_Admin_List_Table')) {
    ...
    class Faulh_Admin_List_Table extends Faulh_Abstract_List_Table {
        ...
        public function get_rows($per_page = 20, $page_number = 1) {
            ...
            $sql = "SELECT"
                    . " FaUserLogin.*, UserMeta.meta_value, ";
        ...
        public function record_count() {
            ...
            //         $sql = "SELECT"
            //         . " COUNT(FaUserLogin.id) AS total, FaUserLogin.*, UserMeta.meta_value, ";
            // $where_query = $this->prepare_where_query();
            // if ($where_query) {
            //     $sql .= $where_query;
            // }
            ...
            if (!empty($_GET['user_status']) || !empty($_GET['p_p_status']) || !empty($_GET['t_t_status']) || !empty($_GET['filter_company']) ) {
                $sql = "SELECT"
                . " COUNT(FaUserLogin.id) AS total,";
                $where_query = $this->prepare_where_query();
                if ($where_query) {
                ...
                $sql = str_replace( [', TIMESTAMPDIFF(SECOND,FaUserLogin.time_login,FaUserLogin.time_last_seen) as duration'], '', $sql );
                // $sql = str_replace( [', TIMESTAMPDIFF(SECOND,FaUserLogin.time_login,FaUserLogin.time_last_seen) as duration', "AND UserMeta.meta_key LIKE  'wp_capabilities'"], '', $sql );
                // $sql .= " WHERE UserMeta.meta_key =  '" . $wpdb->prefix . "capabilities' ";
                $sql .= " WHERE 1";
            }else{
            ...
            // "AND UserMeta.meta_key LIKE  'wp_capabilities'";
            //  die($sql);
            
            // $sql = "SELECT COUNT(FaUserLogin.id) AS total FROM wp_fa_user_logins AS FaUserLogin INNER JOIN wp_usermeta AS UserMeta ON ( UserMeta.user_id=FaUserLogin.user_id AND UserMeta.meta_key LIKE 'wp_capabilities' ) WHERE 1";
    //  $sql = "SELECT COUNT(*) AS 'total_count' FROM wp_fa_user_logins  AS FaUserLogin INNER JOIN wp_usermeta AS UserMeta ON ( UserMeta.user_id=FaUserLogin.user_id ) WHERE UserMeta.meta_key LIKE  'wp_capabilities'";
           // echo $sql;
        ...
        function column_username($item) {
            if (empty($item['user_id'])) {
                ...
                $title = !empty($edit_link) ? "<a href='" . esc_url($edit_link) . "'>" . esc_html($item['username']) . "</a>" : '<strong>' . esc_html($item['username']) . '</strong>';
            }
