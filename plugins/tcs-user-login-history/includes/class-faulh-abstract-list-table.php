...
if (!class_exists('Faulh_Abstract_List_Table')) {
    ...
    abstract class Faulh_Abstract_List_Table extends WP_List_Table {
        ...
        public function prepare_where_query() {
            ...
            if (!empty($_GET['t_t_status'])) {
                $where_query .= " UserMetaStatus.meta_value,";
            }
            
            if (!empty($_GET['p_p_status'])) {
                $where_query .= " UserMetaStatus.meta_value,";
            }
            ...
            $where_query .=" TIMESTAMPDIFF(SECOND,FaUserLogin.time_login,FaUserLogin.time_last_seen) as duration FROM " . $table . "  AS FaUserLogin INNER JOIN $wpdb->usermeta AS UserMeta ON ( UserMeta.user_id=FaUserLogin.user_id AND UserMeta.meta_key LIKE  '" . $wpdb->prefix . "capabilities' )";
            ...
            if (!empty($_GET['user_status'])) {
                    ...
                    $where_query .= " INNER JOIN $wpdb->usermeta AS UserMetaStatus ON ( UserMetaStatus.user_id=FaUserLogin.user_id AND UserMetaStatus.meta_key = 'user_status' ) AND `UserMetaStatus`.`meta_value` = '" . esc_sql($_GET['user_status']) . "'";
            }
            
            if (!empty($_GET['t_t_status'])) {
                    
                    $where_query .= " INNER JOIN $wpdb->usermeta AS UserMetaStatus ON ( UserMetaStatus.user_id=FaUserLogin.user_id AND UserMetaStatus.meta_key = 'tools_templates_access' ) AND `UserMetaStatus`.`meta_value` = '" . esc_sql($_GET['t_t_status']) . "'";
            }
            
            if (!empty($_GET['p_p_status'])) {
                
                    $where_query .= " INNER JOIN $wpdb->usermeta AS UserMetaStatus ON ( UserMetaStatus.user_id=FaUserLogin.user_id AND UserMetaStatus.meta_key = 'policies_procedures_access' ) AND `UserMetaStatus`.`meta_value` = '" . esc_sql($_GET['p_p_status']) . "'";
            }
            ...
            if (!empty($_GET['date_type'])) {
                ...
                if (in_array($date_type, array('all', 'login', 'logout', 'last_seen'))) {
                    ...
                    if (!empty($_GET['date_from']) && !empty($_GET['date_to'])) {
                        ...
                        if ($date_type == 'all') {
                            $where_query .= " AND `FaUserLogin`.`time_login` >= '" . esc_sql($date_from) . "'";
                            $where_query .= " AND `FaUserLogin`.`time_login` <= '" . esc_sql($date_to) . "'";
                        }
                        else {
            ...
            if (!empty($_GET['filter_company'])) {
                ...
                $where_query .= " INNER JOIN $wpdb->usermeta AS UserMetaCompany ON ( UserMetaCompany.user_id=FaUserLogin.user_id AND ( UserMetaCompany.meta_key = 'business_companies' OR UserMetaCompany.meta_key = 'corporate_companies' OR UserMetaCompany.meta_key = 'admin_companies' ) ) AND `UserMetaCompany`.`meta_value` = '" . esc_sql($_GET['filter_company']) . "'";
            }
        ...
        public function column_default($item, $column_name) {
            ...
            switch ($column_name) {
                ...
                case 'username_csv':
                    ...
                    return isset($user_data->user_email) ? esc_html($user_data->user_email) : $this->unknown_symbol;
                ...
                case 'first_name':
                    ...
                    return isset($user_data->first_name) ? esc_html($user_data->first_name) : $this->unknown_symbol;
                ...
                case 'last_name':
                    ...
                    return isset($user_data->last_name) ? esc_html($user_data->last_name) : $this->unknown_symbol;
                ...
                case 'name':
                    ...
                    return isset($name) ? esc_html($name) : $this->unknown_symbol;
                ...
                case 'role':
                    ...
                    elseif (in_array('subscriber_-_corporate', $user_data->roles)) {
                        $user_role = 'Subscriber - Corporate';
                    }
                    elseif (in_array('subscriber_-_admin_facility', $user_data->roles)) {
                        $user_role = 'Subscriber - Admin Facility';
                    }
                    elseif (in_array('subscriber_-_facility_user', $user_data->roles)) {
                        $user_role = 'Subscriber - Facility User';
                    }
                    elseif (in_array('backend-admin', $user_data->roles)) {
                        $user_role = 'Backend Admin';
                    }
                    elseif (in_array('account_manager', $user_data->roles)) {
                    ...
                    return !empty($user_data->roles) ? esc_html($user_role) : $this->unknown_symbol;
                ...
                case 'status':
                    ...
                    if (is_network_admin() && !empty($item['blog_id'])) {
                        ...
                        $status = ucwords(get_user_meta( $item['user_id'], 'user_status', true ));
                    }

                    return isset($status) ? esc_html($status) : $this->unknown_symbol;
                ...
                case 't_t_status':
                    ...
                    if (is_network_admin() && !empty($item['blog_id'])) {
                        ...
                        $t_t_status = ucwords(get_user_meta( $item['user_id'], 'tools_templates_access', true ));
                        
                        if ( empty( $t_t_status ) ) {
                            update_user_meta( $item['user_id'], 'tools_templates_access', 'enable' );
                            $t_t_status = ucwords(get_user_meta( $item['user_id'], 'tools_templates_access', true ));
                        }
                    }
                    
                    return isset($t_t_status) ? esc_html($t_t_status) : $this->unknown_symbol;
                ...
                case 'p_p_status':
                    ...
                    if (is_network_admin() && !empty($item['blog_id'])) {
                        ...
                        $p_p_status = ucwords(get_user_meta( $item['user_id'], 'policies_procedures_access', true ));
                    
                        if ( empty( $p_p_status ) ) {
                            update_user_meta( $item['user_id'], 'policies_procedures_access', 'enable' );
                            $p_p_status = ucwords(get_user_meta( $item['user_id'], 'policies_procedures_access', true ));
                        }
                    }
                    
                    return isset($p_p_status) ? esc_html($p_p_status) : $this->unknown_symbol;
                ...
                case 'business':
                    ...
                    if (is_network_admin() && !empty($item['blog_id'])) {
                        ...
                        $user_business_id = get_the_author_meta( 'business_companies', $item['user_id'] );
                        if (isset($user_business_id) && !empty($user_business_id)) {
                            $business_term = get_term( $user_business_id, 'company' );
                        }
                    }

                    return isset($user_business_id) ? esc_html($business_term->name) : $this->unknown_symbol;
                ...
                case 'business_table':
                    ...
                    return isset($user_business_id) ? $business : $this->unknown_symbol;
                ...
                case 'corporate':
                    ...
                    if (is_network_admin() && !empty($item['blog_id'])) {
                        ...
                        restore_current_blog();
                         ...
                         $user_corporate_id = get_the_author_meta( 'corporate_companies', $item['user_id'] );
                        if (isset($user_corporate_id) && !empty($user_corporate_id)) {
                            $corporate_term = get_term( $user_corporate_id, 'company' );
                        }
                    }

                    return isset($user_corporate_id) ? $corporate_term->name : $this->unknown_symbol;

                case 'corporate_table':
                    if (empty($item['user_id'])) {
                        return $this->unknown_symbol;
                    }
                    ...
                    return isset($user_corporate_id) ? $corporate : $this->unknown_symbol;
                ...
                case 'facility':
                    ...
                    if (is_network_admin() && !empty($item['blog_id'])) {
                        ...
                        restore_current_blog();
                         ...
                         $user_admin_id = get_the_author_meta( 'admin_companies', $item['user_id'] );
                        if (isset($user_admin_id) && !empty($user_admin_id)) {
                            $admin_term = get_term( $user_admin_id, 'company' );
                        }
                    }

                    return isset($user_admin_id) ? $admin_term->name : $this->unknown_symbol;
                ...
                case 'facility_table':
                    ...
                    return isset($user_admin_id) ? $admin : $this->unknown_symbol;
                ...
                case 'account_manager':
                    ...
                    if (is_network_admin() && !empty($item['blog_id'])) {
                        ...
                        $account_manager = get_user_meta( $item['user_id'], 'account_manager', true );
                    }

                    return isset($account_manager) ? esc_html($account_manager) : $this->unknown_symbol;

                case 'account_manager_table':
                    if (empty($item['user_id'])) {
                        return $this->unknown_symbol;
                    }
                    ...
                    return isset($account_manager) ? $account_manager_name : $this->unknown_symbol;
...
//             $per_page = $this->get_items_per_page($this->plugin_name . '_rows_per_page');
        ...
        public function export_to_CSV() {
            ...
            foreach ($data as $row) {
                ...
                if (!$user_id) {
                    $time_last_seen_str = $time_logout_str = $this->unknown_symbol;
                } else {
                ...
                $record[__('Email', 'faulh')] = $this->column_default($row, 'username_csv');
                $record[__('First Name', 'faulh')] = $this->column_default($row, 'first_name');
                $record[__('Last Name', 'faulh')] = $this->column_default($row, 'last_name');
                
                $record[__('User Status', 'faulh')] = $this->column_default($row, 'status');
                $record[__('T&T Access', 'faulh')] = $this->column_default($row, 'p_p_status');
                $record[__('P&P Access', 'faulh')] = $this->column_default($row, 't_t_status');
                $record[__('Role', 'faulh')] = $this->column_default($row, 'role');
                $record[__('Business Member', 'faulh')] = $this->column_default($row, 'business');
                $record[__('Corporate Facility', 'faulh')] = $this->column_default($row, 'corporate');
                $record[__('Admin Facility', 'faulh')] = $this->column_default($row, 'facility');
                $record[__('Account Manager', 'faulh')] = $this->column_default($row, 'account_manager');
                
                //$record[__('Old Role', 'faulh')] = $old_role;
                $record[__('IP Address', 'faulh')] = $this->column_default($row, 'ip_address');

                $record[__('Web Browser', 'faulh')] = $this->column_default($row, 'browser');
