...
if ( ! class_exists( 'WPSC_FAQ_License' ) ) :

	final class WPSC_FAQ_License {
		...
		public static function init() {
			...
			add_action( 'wp_ajax_wpsc_faq_license_activate', array( __CLASS__, 'activate' ) );
			add_action( 'wp_ajax_wpsc_faq_license_deactivate', array( __CLASS__, 'deactivate' ) );
		...
		public static function print_license_body() {
			...
			$license  = isset( $licenses['faq'] ) ? $licenses['faq'] : array();
			...
			<div class="license-container">
				...
				if ( $license_key ) {
					?>
					<button class="wpsc-button small primary" onclick="wpsc_faq_license_deactivate(this, '<?php echo esc_attr( wp_create_nonce( 'wpsc_faq_license_deactivate' ) ); ?>');"><?php echo esc_attr( wpsc__( 'Deactivate' ) ); ?></button>
					<?php
					...
					?>
						<button class="wpsc-button small primary" onclick="wpsc_faq_license_activate(this, '<?php echo esc_attr( wp_create_nonce( 'wpsc_faq_license_activate' ) ); ?>');"><?php echo esc_attr( wpsc__( 'Activate' ) ); ?></button>
		...
		public static function activate() {
			...
			if ( check_ajax_referer( 'wpsc_faq_license_activate', '_ajax_nonce', false ) != 1 ) {
		...
		public static function deactivate() {
			...
			if ( check_ajax_referer( 'wpsc_faq_license_deactivate', '_ajax_nonce', false ) != 1 ) {
				wp_send_json_error( 'Unauthorised request!', 401 );
			}
		...
		public static function load_scripts() {
			...
			<script>
				/**
				 * Activate faq license
				 */
				function wpsc_faq_license_activate(el, nonce) {
					...
					const data = { 
						action: 'wpsc_faq_license_activate',
				 ...
				 * Deactivate license
				 */
				function wpsc_faq_license_deactivate(el, nonce) {
					...
					jQuery(el).text(supportcandy.translations.please_wait);
					const data = { 
						action: 'wpsc_faq_license_deactivate',
						license_key: licenseKey,
						_ajax_nonce: nonce
					};
					jQuery.post(supportcandy.ajax_url, data, function (response) {
