...
$data_attr         = array();
	...
	foreach ( $images as $image ) {
		...
		switch ( $onclick ) {
			...
			case 'link_image':
				$link_start = '<a itemprop="image" class="qode-prettyphoto" href="' . esc_url( $large_img_src ) . '" data-rel="prettyPhoto[rel-' . get_the_ID() . '-' . esc_attr( $gallery_id_rand ) . ']">' . wp_kses_post( $hover_image );
				$link_end = '</a>';
				break;
