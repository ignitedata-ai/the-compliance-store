...
if ( ! class_exists( 'WPSC_CR_Category' ) ) :

	final class WPSC_CR_Category {
		...
		public static function apply_schema() {
			...
			self::$schema = apply_filters( 'wpsc_cr_category_schema', $schema );

			// Prevent modify.
			$prevent_modify       = array( 'id' );
			self::$prevent_modify = apply_filters( 'wpsc_cr_category_prevent_modify', $prevent_modify );
		}
		...
		public function __construct( $id = 0 ) {
			...
			if ( $id > 0 ) {
				...
				$category = $wpdb->get_row( "SELECT * FROM {$wpdb->prefix}psmsc_cr_categories WHERE id = " . $id, ARRAY_A );
				...
				foreach ( $category as $key => $val ) {
					if ( $key == 'name' ) {
						...
						$this->data[ $key ] = $val !== null ? WPSC_Translations::get( 'wpsc-cr-category-' . $category['id'], stripslashes( $val ) ) : '';
		...
		public function save() {
			...
			WPSC_Translations::add( 'wpsc-cr-category-' . $this->data['id'], $this->data['name'] );
		...
		public static function insert( $data ) {
			...
			// string translation.
			WPSC_Translations::add( 'wpsc-cr-category-' . $id, $data['name'] );
		 ...
		 *
		 * @param WPSC_CR_Category $category - category object.
		 * @return boolean
		 */
		public static function destroy( $category ) {
			...
			WPSC_Translations::remove( 'wpsc-cr-category-' . $category->id );
			return true;
		}
		...
		private function set_data( $data ) {
			...
			foreach ( $data as $var_name => $val ) {
				if ( $var_name == 'name' ) {
					...
					$this->data[ $var_name ] = $val !== null ? WPSC_Translations::get( 'wpsc-cr-category-' . $data['id'], stripslashes( $val ) ) : '';
		...
		public static function find( $filter = array(), $is_object = true ) {
			...
			$sql   = 'SELECT * FROM ' . $wpdb->prefix . 'psmsc_cr_categories ';
			...
			// total results.
			$sql = 'SELECT count(id) FROM ' . $wpdb->prefix . 'psmsc_cr_categories ';
		...
		public static function load_ref_class( $classes ) {
			...
			$classes['wpsc_cr_category'] = array(
				'class'    => __CLASS__,
				'save-key' => 'id',
			);
			return $classes;
