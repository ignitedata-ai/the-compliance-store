...
if ( ! class_exists( 'WPSC_Canned_Reply_Model' ) ) :

	final class WPSC_Canned_Reply_Model {
		...
		public static function apply_schema() {
			...
			self::$prevent_modify = apply_filters( 'wpsc_canned_replies_prevent_modify', $prevent_modify );
		}
		...
		public function __construct( $id = 0 ) {
			...
			if ( $id > 0 ) {
				...
				$creply = $wpdb->get_row( "SELECT * FROM {$wpdb->prefix}psmsc_canned_reply WHERE id = " . $id, ARRAY_A );
				if ( ! is_array( $creply ) ) {
					return;
				}

				foreach ( $creply as $key => $val ) {
		 ...
		 * @return WPSC_Canned_Reply_Model
		 */
		public static function insert( $data ) {
			...
			$cr = new WPSC_Canned_Reply_Model( $wpdb->insert_id );
			return $cr;
		}
		 ...
		 *
		 * @param WPSC_Canned_Reply_Model $creply - canned reply object.
		 * @return boolean
		 */
		public static function destroy( $creply ) {
		...
		public static function find( $filter = array(), $is_object = true ) {
			...
			$sql   = 'SELECT * FROM ' . $wpdb->prefix . 'psmsc_canned_reply ';
			...
			// total results.
			$sql = 'SELECT count(id) FROM ' . $wpdb->prefix . 'psmsc_canned_reply ';
			...
			$temp_results = array();
			foreach ( $response['results'] as $cr ) {

				$ob   = new WPSC_Canned_Reply_Model();
				$data = array();
		...
		public static function load_ref_class( $classes ) {
			...
			$classes['wpsc_canned_replies'] = array(
				'class'    => __CLASS__,
				'save-key' => 'id',
			);
			return $classes;
