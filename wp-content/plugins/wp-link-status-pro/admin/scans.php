...
class WPLNST_Admin_Scans {
	...
	private function scans_results($scan_id) {
		...
		$export_results = $this->admin->scans->get_scan_results_without_pagination(array(
			'scan_id' 				=> $scan->id,
			'status_code' 			=> $status_code,
			'status_level' 			=> $status_level,
			'object_type'			=> $object_type,
			...
			'search_anchor_type' 	=> $search_anchor_type,
			'order_date'			=> $scan->crawl_order,
			'total'					=>$scans->rows[0]->summary['status_total'],
			/*'paged' 				=> empty($_GET['paged'])? 0 : (int) $_GET['paged'],*/
		));

/*    	echo "<pre>";
    	//var_dump($scans->rows[0]->row->scan_id);
    	//var_dump($scans->rows[0]->summary['status_total']);
    	var_dump($export_results);
    	echo "</pre>";
    	exit();*/
		// Unknown error
		if (false === $results) {
		...
		$export_results->is_search = (false !== $object_type) || (false !== $object_post_type) || (false !== $link_type) || (false !== $ignored_type) || (false !== $seo_link_type) || (false !== $protocol_type) || (false !== $special_type) || (false !== $action_type) || (false !== $dest_type) || (false !== $search_url) || (false !== $search_anchor);
		$results->is_all_results = (false === $status_code) && (false === $status_level) && !$results->is_search;
		$export_results->is_all_results = (false === $status_code) && (false === $status_level) && !$export_results->is_search;
		...
		$csv_file_path = plugin_dir_path( __DIR__ ) . "reports-links-logs.csv";

    	$fp = fopen($csv_file_path, 'w');

    	$header_names = array("URL ID", "URL", "Status", "Content", "External Or Internal");
    	fputcsv($fp, $header_names);
    	foreach ($export_results->rows as $result) {
    		//var_dump($result);
	        $fields = array($result->url_id,$result->url,$result->status_code, get_the_title($result->object_id), $result->scope );
	        fputcsv($fp, $fields);
	    }

	    fclose($fp);

	    $file_url = plugins_url("wp-link-status-pro/reports-links-logs.csv");
	    $GLOBALS['file_url'] = $file_url;
