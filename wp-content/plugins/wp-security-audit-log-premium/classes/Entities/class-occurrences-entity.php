...
if ( ! class_exists( '\WSAL\Entities\Occurrences_Entity' ) ) {
	...
	class Occurrences_Entity extends Abstract_Entity {
		...
		// phpcs:enable
		/**
		 * Returns prepared with filters URL for the past 24 hours based on the type of the link provided.
		 *
		 * @param string $type - The type of URL to generate. Possible types are 'total', 'logins', 'changes'. Total generates link for all events, logins generates link for logins and changes generates link for all the changes for plugins and / or themes.
		 *
		 * @return string|void
		 *
		 * @since 5.4.2
		 */
		public static function get_last24_links( string $type = 'total' ) {
			$date_obj = new \DateTime();
			$date_obj->setTime( 0, 0 ); // Set time of the object to 00:00:00.
			$date_string = $date_obj->format( 'U' ); // Get the date in UNIX timestamp.
			...
			$report_end_date   = gmdate( 'Y-m-d', $end_date );
			$report_start_date = gmdate( 'Y-m-d', $start_date );
			...
			if ( $report_end_date ) {
				$filters_string .= '&filters%5B' . ( ++$key ) . '%5D=from%3A' . $report_start_date;
				$filters_string .= '&filters%5B' . ( ++$key ) . '%5D=to%3A' . $report_end_date;
			} else {
				$filters_string .= '&filters%5B' . ( ++$key ) . '%5D=on%3A' . $report_start_date;
			}

			if ( \in_array( $type, array( 'total', 'logins', 'changes' ), true ) ) {
				if ( 'logins' === $type ) {
					$login_events_ids = array( 1000, 1005 );
					foreach ( $login_events_ids as $event_id ) {
						$filters_string .= '&filters%5B' . ( ++$key ) . '%5D=event%3A' . $event_id;
					}
				}
				if ( 'changes' === $type ) {
					$plugin_ids = array( 5000, 5001, 5002, 5003, 5004, 5005, 5006, 5007, 5030, 5031 );
					foreach ( $plugin_ids as $event_id ) {
						$filters_string .= '&filters%5B' . ( ++$key ) . '%5D=event%3A' . $event_id;
					}
				}
			} else {
			...
			$link = \WpSecurityAuditLog::get_plugin_admin_url_page() . '&wsal-cbid=-1' . $filters_string;
