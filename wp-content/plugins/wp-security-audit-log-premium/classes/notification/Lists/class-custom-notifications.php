...
// phpcs:enable
		...
		public static function format_column_value( $item, $column_name ) {
			$item['settings'] = array();
			if ( isset( $item['notification_settings'] ) ) {
				$item['settings'] = \json_decode( $item['notification_settings'], true );
			}
			...
			switch ( $column_name ) {
				...
				case 'notification_phone':
					$phone = Notification_Helper::get_custom_notification_phone( $item );

					return ( $phone ) ? $phone : __( 'No phone is set', 'wp-security-audit-log' );
				case 'notification_slack':
					$channel = Notification_Helper::get_custom_notification_channel( $item );

					return ( $channel ) ? $channel : __( 'No channel is set', 'wp-security-audit-log' );
				case 'notification_email':
					$email = Notification_Helper::get_custom_notification_email( $item, 0 );

					return ( $email ) ? $email : __( 'No default address is set', 'wp-security-audit-log' );
				case 'created_on':
