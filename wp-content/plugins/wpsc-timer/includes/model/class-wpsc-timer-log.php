...
if ( ! class_exists( 'WPSC_Timer_Log' ) ) :

	final class WPSC_Timer_Log {
		...
		public static function init() {
			...
			// Delete tickets logs.
			add_action( 'wpsc_ticket_delete_permanently', array( __CLASS__, 'delete_timer_logs' ) );
		}
		...
		public static function apply_schema() {
			...
			self::$schema = apply_filters( 'wpsc_timer_log_schema', $schema );

			// Prevent modify.
			$prevent_modify       = array( 'id' );
			self::$prevent_modify = apply_filters( 'wpsc_timer_log_prevent_modify', $prevent_modify );
		}
		...
		public function __construct( $id = 0 ) {
			...
			if ( $id > 0 ) {
				...
				$log = $wpdb->get_row( "SELECT * FROM {$wpdb->prefix}psmsc_timer_logs WHERE id = " . $id, ARRAY_A );
				if ( ! is_array( $log ) ) {
					return;
				}
		...
		public static function insert( $data ) {
			...
			$timer_log = new WPSC_Timer_Log( $wpdb->insert_id );
			return $timer_log;
		}
		 ...
		 *
		 * @param WPSC_Timer_Log $log - timer log object.
		 * @return boolean
		 */
		...
		public static function find( $filter = array(), $is_object = true ) {
			...
			$sql   = 'SELECT * FROM ' . $wpdb->prefix . 'psmsc_timer_logs ';
			...
			// total results.
			$sql = 'SELECT count(id) FROM ' . $wpdb->prefix . 'psmsc_timer_logs ';
		...
		private static function get_where( $filter ) {
			...
			$meta_query = apply_filters( 'wpsc_timer_log_meta_query', $meta_query, $filter );
			if ( $meta_query ) {
		...
		/**
		 * Delete timer logs after a ticket is permanently delete
		 *
		 * @param integer $ticket_id - ticket id.
		 * @return void
		 */
		public static function delete_timer_logs( $ticket_id ) {
