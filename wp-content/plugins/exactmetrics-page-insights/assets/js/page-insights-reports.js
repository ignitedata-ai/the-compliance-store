function ExactMetrics_Posts_Reports() {

  var self = this,
    $ = window.jQuery,
    posts_table = document.querySelector('.wp-list-table'),
    page_body = document.querySelector('body'),
    reports_overlay = document.querySelector('.exactmetrics-reports-overlay'),
    $reports_overlay = $(reports_overlay),
    overlay_title = reports_overlay.querySelector('.exactmetrics-reports-overlay-title-text'),
    overlay_content = reports_overlay.querySelector('.exactmetrics-reports-overlay-content'),
    close_button = reports_overlay.querySelector('.exactmetrics-close-overlay'),
    report_interval = reports_overlay.querySelector('#exactmetrics-report-interval'),
    load_element = overlay_content.innerHTML,
    $error_template = $(document.getElementById('exactmetrics-pageinsights-error-template')).html();
  ...
  this.add_events = function () {
    ...
    posts_table.addEventListener('click', function (event) {
      var el = event.target;
      if (el.classList && el.classList.contains('exactmetrics-reports-loader')) {
    ...
    $reports_overlay.on('click.exactmetrics', function (event) {
  ...
  this.show_overlay = function () {
    page_body.classList.add('exactmetrics-reports-overlay-visible');
    report_interval.focus();
    self.add_close_handlers();
  };

  this.close_overlay = function () {
    page_body.classList.remove('exactmetrics-reports-overlay-visible');
    self.remove_close_handlers();

    var element = document.querySelector('.exactmetrics-reports-loader[data-post_id="' + this.get('post_id') + '"]');
    element.focus();
  };
  ...
  this.add_close_handlers = function () {
    $(document).on('keyup.exactmetrics', function (e) {
      if (27 === e.keyCode) {
        self.close_overlay();
      }
    });
  ...
  this.remove_close_handlers = function () {
    $(document).off('keyup.exactmetrics');
  };
  ...
  this.grab_report_content = function () {
    ...
    $.ajax({
      url: ajaxurl,
      data: {
        action: 'exactmetrics_pageinsights_refresh_report',
      ...
      if (response.success && response.data.html) {
        ...
        $(overlay_content).html(self.error_markup(exactmetrics_page_insights_admin.error_text, text, false));
      }
      ...
      $(overlay_content).html(self.error_markup(exactmetrics_page_insights_admin.error_text, error_details_text, false));
    });
  };
  ...
  this.error_markup = function (title, text, footer) {
    ...
    error_template.find('.exactmetrics-pageinsights-error-title').html(title);
    error_template.find('.exactmetrics-pageinsights-error-content').html(text);

    if (footer) {
      error_template.find('.exactmetrics-pageinsights-error-footer').html(footer).addClass('visible');
    }
  ...
  this.check_empty_column = function () {
    ...
    var column_data = $('.exactmetrics_reports.column-exactmetrics_reports');
    ...
    if (delete_column) {
      $('.column-exactmetrics_reports').remove();
    }
  };
  ...
  this.start_more_polling = function () {
    $.ajax({
      url: ajaxurl,
      data: {
        action: 'exactmetrics_pageinsights_check_background_progress',
        security: exactmetrics_page_insights_admin.admin_nonce,
      }
    }).done(function (response) {
      if (response.success) {
...
new ExactMetrics_Posts_Reports();
