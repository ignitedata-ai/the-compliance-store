...
use ExactMetrics_API_Request;
use ExactMetrics_Rest_Routes;
use WP_Error;
 ...
 */
class ExactMetrics_AI_Insights
{
	/**
	 * The transient key for the AI insights
	 */
	const INSIGHTS_TRANSIENT_KEY = 'exactmetrics_ai_insights';

	/**
	 * The option key for the AI insights rating
	 */
	const INSIGHTS_RATING_OPTION_KEY = 'exactmetrics_ai_insights_rating';

	/**
	 * Name of the option used to make sure we don't send the same feedback data multiple times
	 */
	const INSIGHTS_FEEDBACK_CHECKIN_LIST_KEY = 'exactmetrics_ai_insights_feedback_checkin_list';

	/**
	 * The transient key for the list of addons for AI
	 */
	const ADDONS_FOR_AI_TRANSIENT_KEY = 'exactmetrics_addons_for_ai';

	/**
	 * The transient key for the list of addons for AI
	 */
	const AI_INSIGHTS_READ_OPTION_KEY = 'exactmetrics_ai_insights_read';
	...
	private function init_hooks()
	{
		...
		//  Cron
		add_action( 'exactmetrics_ai_insights_checkin', [$this, 'ratings_and_feedback_checkin'] );

		if ( ! wp_next_scheduled( 'exactmetrics_ai_insights_checkin' ) ) {
			wp_schedule_event( time(), 'daily', 'exactmetrics_ai_insights_checkin' );
		}

		//	Clear addons cache when addon is activated or deactivated
		add_action( 'exactmetrics_after_ajax_activate_addon', [$this, 'clear_cached_addons_for_ai'] );
		add_action( 'exactmetrics_after_ajax_deactivate_addon', [$this, 'clear_cached_addons_for_ai'] );
	}
	...
	{
		register_rest_route( 'exactmetrics/v1', '/ai-insights', array(
			'methods'               => WP_REST_Server::READABLE,
			'callback'              => [$this, 'get_ai_insights'],
			'permission_callback'   => [$this, 'ai_insights_permissions_callback'],
		));

		register_rest_route( 'exactmetrics/v1', '/ai-insights/ratings', array(
			'methods'               => WP_REST_Server::READABLE,
			'callback'              => [$this, 'get_current_insights_ratings'],
			'permission_callback'   => [$this, 'ai_insights_permissions_callback'],
		));

		register_rest_route( 'exactmetrics/v1', '/ai-insights/rate', array(
			'methods'               => WP_REST_Server::CREATABLE,
			'callback'              => [$this, 'rate_insight'],
			'permission_callback'   => [$this, 'ai_insights_permissions_callback']
		));

		register_rest_route( 'exactmetrics/v1', '/ai-insights/feedback', array(
			'methods'               => WP_REST_Server::CREATABLE,
	...
	public function ratings_and_feedback_checkin()
	{
		...
		$api = new ExactMetrics_API_Request( 'analytics/ai/feedback', [], 'POST' );
	...
	public static function check_license()
	{
		if ( ! ExactMetrics()->license->is_site_licensed() && ! ExactMetrics()->license->is_network_licensed() ) {
			...
			$message = sprintf(
				esc_html__( 'Oops! You cannot view AI Insights and Conversations because you are not licensed. %1$sAdd your license%2$s. If the issue continues, please %3$scontact our support%4$s team.', 'exactmetrics-premium' ),
		...
		foreach ( $addons_data as $addons_type => $addons ) {
			foreach ( $addons as $addon ) {
				...
				if ( 'exactmetrics-ecommerce' === $slug && 'm' === $slug[0] ) {
					$addon = exactmetrics_get_addon( $installed_plugins, $addons_type, $addon, $slug );
	...
	public static function generate_new_insights()
	{
		...
		$api = new ExactMetrics_API_Request( 'analytics/ai/insights', [], 'GET' );
		...
		if ( is_wp_error( $result ) ) {
			if ( 429 === $result->get_error_code() ) {
				$message = sprintf(
					esc_html__( 'You\'ve exceeded the number of daily AI Insights allowed. Please try again tomorrow. If you have questions please %1$scontact our support%2$s team.', 'exactmetrics-premium' ),
					'<a target="_blank" href="' . exactmetrics_get_url( 'notice', 'ai-quota', 'https://www.exactmetrics.com/my-account/support/' ) . '">',
					'</a>'
				);
		...
		if ( empty( $result ) ) {
			return [
				'success' => false,
				'error'   => __('There was an issue connecting to the AI service, please try again.', 'exactmetrics-premium'),
				'data'    => [],
			];
		}
