...
/**
 * Class ExactMetrics_Popular_Posts_Products_Sidebar
 */
class ExactMetrics_Popular_Posts_Products_Sidebar extends WP_Widget {
	/**
	...
	public function __construct() {
		...
		// Widget Slug.
		$widget_slug = 'exactmetrics-popular-posts-products';

		// Widget basics.
		$widget_ops = array(
			'classname'   => $widget_slug,
			'description' => esc_html_x( 'Display popular products.', 'Widget', 'exactmetrics-premium' ),
		);
		...
		// Load widget.
		parent::__construct( $widget_slug, esc_html_x( 'Popular Products - ExactMetrics', 'Widget', 'exactmetrics-premium' ), $widget_ops, $control_ops );
	}
	...
	public function widget( $args, $instance ) {
		...
		echo ExactMetrics_Popular_Posts_Products()->shortcode_output( $atts ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	...
	public function update( $new_instance, $old_instance ) {
		...
		$themes = new ExactMetrics_Popular_Posts_Themes( 'products', ! empty( $old_instance['theme'] ) ? $old_instance['theme'] : '' );
	...
	public function form( $instance ) {
		...
		$themes     = new ExactMetrics_Popular_Posts_Themes( 'products', $theme_name );
		$theme      = $themes->get_theme();
		$this->prepare_defaults_from_theme( $theme );

		$instance = wp_parse_args( (array) $instance, $this->defaults );

		$title_font_sizes = apply_filters( 'exactmetrics_popular_posts_products_title_sizes', range( 10, 35 ) );
		$meta_font_sizes  = apply_filters( 'exactmetrics_popular_posts_products_meta_sizes', range( 8, 24 ) );

		$this->text_input( 'title', _x( 'Widget Title:', 'Widget', 'exactmetrics-premium' ), $instance );
		?>
		<p>
			<input type="checkbox"
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'display_title' ) ); ?>">
				<?php echo esc_html( _x( 'Display Widget Title', 'Widget', 'exactmetrics-premium' ) ); ?>
			</label>
		</p>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'post_count' ) ); ?>">
				<?php echo esc_html( _x( 'Number of posts to display:', 'Widget', 'exactmetrics-premium' ) ); ?>
			</label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'theme' ) ); ?>">
				<?php echo esc_html( _x( 'Theme:', 'Widget', 'exactmetrics-premium' ) ); ?>
			</label>
		...
		<div class="exactmetrics-widget-theme-preview">
			<span class="exactmetrics-widget-theme-preview-label">
				<?php esc_html_e( 'Theme Preview', 'exactmetrics-premium' ); ?>
			</span>
			<div
				class="exactmetrics-products-theme-preview-icon exactmetrics-products-theme-preview-icon-<?php echo esc_attr( $instance['theme'] ); ?>"></div>
		</div>
		<?php if ( ! empty( $theme['styles']['title']['color'] ) ) {
			$this->color_input( 'title_color', _x( 'Title Color:', 'Widget', 'exactmetrics-premium' ), $instance );
			?>
			<?php
		}
		if ( ! empty( $theme['styles']['title']['size'] ) ) {
			$this->size_input( 'title_size', _x( 'Title Font Size:', 'Widget', 'exactmetrics-premium' ), $instance, $title_font_sizes );
		}
		if ( ! empty( $theme['styles']['price']['color'] ) ) {
			$this->color_input( 'price_color', _x( 'Price Color:', 'Widget', 'exactmetrics-premium' ), $instance );
		}
		if ( ! empty( $theme['styles']['price']['size'] ) ) {
			$this->size_input( 'price_size', _x( 'Price Font Size:', 'Widget', 'exactmetrics-premium' ), $instance, $meta_font_sizes );
		}
		if ( ! empty( $theme['styles']['rating']['color'] ) ) {
			$this->color_input( 'rating_color', _x( 'Rating Color:', 'Widget', 'exactmetrics-premium' ), $instance );
		}
		if ( ! empty( $theme['styles']['background']['border'] ) ) {
			$this->color_input( 'background_border', _x( 'Border Color:', 'Widget', 'exactmetrics-premium' ), $instance );
		}
		if ( ! empty( $theme['styles']['background']['color'] ) ) {
			$this->color_input( 'background_color', _x( 'Background Color:', 'Widget', 'exactmetrics-premium' ), $instance );
		}
		?>
		<p>

			<label class="exactmetrics-label-block">
				<?php echo esc_html( _x( 'Only Show Products from These Categories:', 'Widget', 'exactmetrics-premium' ) ); ?>
			</label>
			<select name="<?php echo esc_attr( $this->get_field_name( 'categories' ) ); ?>[]"
					class="exactmetrics-multiselect" data-taxonomy="product_cat" multiple>
				<?php
		...
		<p>
			<?php if ( ! empty( $theme['styles']['meta']['price'] ) ) { ?>
				<label class="exactmetrics-label-block">
					...
					<?php esc_html_e( 'Display Price', 'exactmetrics-premium' ); ?>
				</label>
			<?php } ?>
			<?php if ( ! empty( $theme['styles']['meta']['rating'] ) ) { ?>
				<label class="exactmetrics-label-block">
					...
					<?php esc_html_e( 'Display Rating', 'exactmetrics-premium' ); ?>
				</label>
			<?php } ?>
			<?php if ( ! empty( $theme['styles']['meta']['image'] ) ) { ?>
				<label class="exactmetrics-label-block">
					...
					<?php esc_html_e( 'Display Image', 'exactmetrics-premium' ); ?>
				</label>
			<?php } ?>
		</p>
		<?php
		...
		<p>
			...
			<select id="<?php echo esc_attr( $this->get_field_id( $name ) ); ?>"
				...
				<?php foreach ( $range as $font_size ) { ?>
					<option
						value="<?php echo absint( $font_size ); ?>" <?php selected( $instance[ $name ], $font_size ); ?>><?php printf( esc_html_x( '%dpx', 'exactmetrics-premium' ), $font_size ); // phpcs:ignore ?></option>
				<?php } ?>
			</select>
		</p>
		<?php
