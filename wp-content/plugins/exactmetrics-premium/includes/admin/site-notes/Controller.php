...
class ExactMetrics_SiteNotes_Controller {
	...
	public function get_notes() {
		...
		if ( ! current_user_can( 'exactmetrics_view_dashboard' ) && ! current_user_can( 'exactmetrics_save_settings' ) ) {
			wp_send_json(
				array(
					'published' => false,
					'message' => __( "You don't have permission to view notes.", 'exactmetrics-premium' ),
				)
			);
		}
	...
	public function get_categories() {
		...
		if ( ! current_user_can( 'exactmetrics_view_dashboard' ) && ! current_user_can( 'exactmetrics_save_settings' ) ) {
			wp_send_json(
				array(
					'published' => false,
					'message' => __( "You don't have permission to view notes categories.", 'exactmetrics-premium' ),
				)
			);
		}
	...
	public function save_note() {
		...
		if ( ! current_user_can( 'exactmetrics_save_settings' ) ) {
			wp_send_json(
				array(
					'published' => false,
					'message' => __( "You don't have permission to update notes.", 'exactmetrics-premium' ),
				)
			);
		}
	...
	public function save_category() {
		...
		if ( ! current_user_can( 'exactmetrics_save_settings' ) ) {
			wp_send_json(
				array(
					'published' => false,
					'message' => __( "You don't have permission to update categories.", 'exactmetrics-premium' ),
				)
			);
		}
		...
		if (empty($category->name)) {
			wp_send_json(
				array(
					'published' => false,
					'message' => __('Please add a category name', 'exactmetrics-premium'),
				)
			);
		}
		...
		if (200 < mb_strlen($category->name)) {
			wp_send_json(
				array(
					'published' => false,
					'message' => __('You can\'t exceed the 200 characters length for each site note.', 'exactmetrics-premium'),
				)
			);
		}
	...
	public function trash_notes() {
		...
		if (empty($ids)) {
			wp_send_json(
				array(
					'success' => false,
					'message' => __('Please choose a site note to trash!', 'exactmetrics-premium'),
				)
			);
		}
	...
	public function restore_notes() {
		...
		if (empty($ids)) {
			wp_send_json(
				array(
					'success' => false,
					'message' => __('Please choose a site note(s) to restore!', 'exactmetrics-premium'),
				)
			);
		}
	...
	public function delete_notes() {
		...
		if (empty($ids)) {
			wp_send_json(
				array(
					'success' => false,
					'message' => __('Please choose a site note(s) to delete!', 'exactmetrics-premium'),
				)
			);
		}
	...
	public function delete_categories() {
		...
		if (empty($ids)) {
			wp_send_json(
				array(
					'success' => false,
					'message' => __('Please choose a category to delete!', 'exactmetrics-premium'),
				)
			);
		}
	...
	public function export_notes() {
		...
		$headers = array(
			__('Date', 'exactmetrics-premium'),
			__('Site Note', 'exactmetrics-premium'),
			__('Category', 'exactmetrics-premium'),
		...
		foreach ($items['items'] as $item) {
			$item_media = __('NA', 'exactmetrics-premium');
	...
	public function export_notes_to_ga4() {
		...
		if ( empty( $annotations ) ) {
			wp_send_json_error(
				array(
					'message' => __( 'No annotations data provided.', 'exactmetrics-premium' ),
				)
			);
		}
		...
		if (
			...
			wp_send_json_error(
				array(
					'message' => __( 'You must be properly authenticated with ExactMetrics to export annotations.', 'exactmetrics-premium' ),
				)
			);
		}
		...
		wp_send_json_success(
			array(
				'message' => __( 'Annotations exported successfully.', 'exactmetrics-premium' ),
				'data'    => $response,
			)
		);
	}
	...
	public function deleted_note_from_ga4_single($ga4_annotation_id) {
		...
		if (!(ExactMetrics()->auth->is_authed() || ExactMetrics()->auth->is_network_authed())) {
			return new WP_Error('not_authenticated', __('You must be properly authenticated with ExactMetrics to delete annotations.', 'exactmetrics-premium'));
		}
	...
	public function import_notes_from_ga4() {
		...
		if ( $skipped_count > 0 ) {
			$message .= ' ' . sprintf(
				__( '%d annotations were skipped (already exist).', 'exactmetrics-premium' ),
				$skipped_count
			);
		}
		...
		<div class="exactmetrics-metabox" id="exactmetrics-metabox-site-notes">
			<div class="exactmetrics-metabox-input exactmetrics-metabox-input-checkbox">
				<label class="">
					...
					<span class="exactmetrics-metabox-input-checkbox-label"><?php _e('Add a Site Note', 'exactmetrics-premium'); ?></span>
				</label>
			</div>
			...
			<div id="site-notes-active-container" class="<?php echo (!$sitenote_active ? 'hidden' : ''); ?>">
				...
				<div class="exactmetrics-metabox-input exactmetrics-metabox-select">
					<label>
						<?php _e('Category', 'exactmetrics-premium'); ?>
