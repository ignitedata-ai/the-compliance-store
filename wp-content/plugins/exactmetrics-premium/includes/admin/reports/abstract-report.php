...
class ExactMetrics_Report {
	...
	public function show_report( $args = array() ) {
		...
		if ( ! current_user_can( 'exactmetrics_view_dashboard' ) ) {
			return exactmetrics_get_message( 'error', esc_html__( 'Oops! Access is denied. If you believe you should be able to view this report, please contact your website administrator to ensure you have the correct user role to view ExactMetrics reports.', 'exactmetrics-premium' ) );
		}
		...
		if ( exactmetrics_get_option( 'dashboard_disabled', false ) ) {
			if ( current_user_can( 'exactmetrics_save_settings' ) ) {
				...
				return exactmetrics_get_message( 'error', sprintf( esc_html__( 'Please %1$senable the dashboard%2$s to see report data.', 'exactmetrics-premium' ), '<a href="' . $url . '">', '</a>' ) );
			} else {
				$message = sprintf(
				// Translators: Link tag starts with url and link tag ends.
					esc_html__( 'Oops! The ExactMetrics dashboard has been disabled. Please check with your site administrator that your role is included in the ExactMetrics permissions settings. %1$sClick here for more information%2$s.', 'exactmetrics-premium' ),
		...
		if ( exactmetrics_is_pro_version() ) {
			if ( ! ExactMetrics()->license->has_license() ) {
				...
				$message = sprintf(
					esc_html__( 'Oops! We did not find an active ExactMetrics license. Please %1$scheck your license settings%2$s or %3$scontact our support team%4$s for help.', 'exactmetrics-premium' ),
					'<a href="' . $url . '">',
					'</a>',
		...
		if ( ! ( ExactMetrics()->auth->is_authed() || ExactMetrics()->auth->is_network_authed() ) ) {
			if ( current_user_can( 'exactmetrics_save_settings' ) ) {
				...
				// Translators: Placeholders add a link to the settings panel.
				return exactmetrics_get_message( 'error', sprintf( esc_html__( 'Oops! We did not find a properly authenticated analytics account. Please %1$sauthenticate with Google%2$s to allow ExactMetrics to show you reports.', 'exactmetrics-premium' ), '<a href="' . $url . '">', '</a>' ) );
			} else {
				return exactmetrics_get_message( 'error', esc_html__( 'Oops! It appears as though you do not have the right user permissions to authenticate. Please contact your website administrator to check your user roles.', 'exactmetrics-premium' ) );
			}
		}
		...
		if ( empty( $args['data'] ) || ! is_array( $args['data'] ) ) {
			if ( exactmetrics_is_pro_version() ) {
				...
				if ( empty( $args['data'] ) || is_array( $args['data'] ) ) {
					return exactmetrics_get_message( 'error', __( 'No data found', 'exactmetrics-premium' ) );
				}
	...
	public function get_data( $args = array() ) {
		...
		if ( ! $this->is_valid_date_range( $start, $end ) ) {
			return array(
				'success' => false,
				'error'   => __( 'Whoops! No data found for this date range', 'exactmetrics-premium' ),
				'data'    => array(
		...
		if ( ( $start !== $this->default_start_date() || $end !== $this->default_end_date() ) && ! exactmetrics_is_pro_version() ) {
			...
			// return array(
			// 	'success' => false,
			// 	'error'   => __( 'Please upgrade to ExactMetrics Pro to use custom date ranges.', 'exactmetrics-premium' ),
			// 	'data'    => array(),
			// );
		}
		...
		if ( $site_auth || $ms_auth ) {
			...
			return array(
				...
				'error'   => sprintf(
					...
					__( 'You must be properly authenticated with ExactMetrics to use our reports. Please use our %1$ssetup wizard%2$s to get started.', 'exactmetrics-premium' ),
					'<a href=" ' . $url . ' ">',
					'</a>'
				),
				'data'    => array(),
	...
	public function get_upsell_notice() {
		...
		$message = sprintf( __( 'You currently have a %1$s level license, but this report requires at least a %2$s level license to view the %3$s. Please upgrade to view this report.', 'exactmetrics-premium' ), $has_level, $this->level, $this->title );
		ob_start(); ?>
		<div
					...
					<div class="col-lg-10 col-lg-offset-1 align-self-center">
						<div class="exactmetrics-upsell-card">
							...
							<div class="exactmetrics-upsell-card-card-content">
								<span
									class="exactmetrics-upsell-card-title"><?php esc_html_e( 'Ready to Get Analytics Super-Powers?', 'exactmetrics-premium' ); ?></span>
								<p class="exactmetrics-upsell-card-subtitle">
									<strong><?php esc_html_e( '(And Crush Your Competition?)', 'exactmetrics-premium' ); ?></strong>
								</p> &nbsp;
								<?php if ( exactmetrics_is_pro_version() ) { ?>
									<p>
										<?php
										// Translators: License level and smiley.
										echo sprintf( esc_html__( 'Hey there! It looks like you\'ve got the %1$s license installed on your site. That\'s awesome! %s', 'exactmetrics-premium' ), $has_level, '<span class="dashicons dashicons-smiley"></span>' ); // phpcs:ignore
										?>
									</p>
									&nbsp;
									<p>
										...
										echo sprintf( esc_html__( 'Do you want to access to %1$s reporting right now%2$s in your WordPress Dashboard? That comes with the %3$s level%4$s of our paid packages. You\'ll need to upgrade your license to get instant access.', 'exactmetrics-premium' ), '<strong>' . $this->title, '</strong>', '<strong><a href="' . exactmetrics_get_url( 'reports-page', $this->name . '-report-upsell-license-link', 'https://exactmetrics.com/my-account/' ) . '">' . $this->level, '</a></strong>' ); // phpcs:ignore
										?>
									</p>
									&nbsp;
									<p>
										...
										echo sprintf( esc_html__( 'It\'s easy! To upgrade, navigate to %1$sMy Account%2$s on ExactMetrics.com, go to the licenses tab, and click upgrade. We also have a %3$sstep by step guide%4$s with pictures of this process.', 'exactmetrics-premium' ), '<a href="' . exactmetrics_get_url( 'reports-page', $this->name . '-report-upsell-license-link', 'https://exactmetrics.com/my-account/' ) . '"><strong>', '</strong></a>', '<a href="' . exactmetrics_get_url( 'reports-page', $this->name . '-report-upsell-license-link', 'https://www.exactmetrics.com/docs/upgrade-exactmetrics-license/' ) . '" style="text-decoration:underline !important">', '</a>' ); // phpcs:ignore
										?>
									</p>
									&nbsp;
									<p><?php esc_html_e( 'If you have any questions, don\'t hesitate to reach out. We\'re here to help.', 'exactmetrics-premium' ); ?></p>
								<?php } else { ?>
									<p>
										<?php
										// Translators: Placeholder adds a smiley face.
										echo sprintf( esc_html__( 'Hey there! %s It looks like you\'ve got the free version of ExactMetrics installed on your site. That\'s awesome!', 'exactmetrics-premium' ), '<span class="dashicons dashicons-smiley"></span>' );
										?>
									</p>
									&nbsp;
									<p>
										...
										echo sprintf( esc_html__( 'Do you you want to access to %1$s reporting right now%2$s in your WordPress Dashboard? That comes with %3$s level%4$s of our paid packages. To get instant access, you\'ll want to buy a ExactMetrics license, which also gives you access to powerful addons, expanded reporting (including the ability to use custom date ranges), comprehensive tracking features (like UserID tracking) and access to our world-class support team.', 'exactmetrics-premium' ), '<strong>' . $this->title, '</strong>', '<a href="' . exactmetrics_get_upgrade_link( 'reports-page', $this->name . '-report-upsell-license-link' ) . '">' . $this->level, '</a>' ); // phpcs:ignore
										?>
									</p>
									&nbsp;
									<p>
										<?php
										// Translators: Placeholders make the text bold, add the license level and add a link to upgrade.
										echo sprintf( esc_html__( 'Upgrading is easy! To upgrade, navigate to %1$ssour pricing page%2$s, purchase the required license, and then follow the %3$sinstructions in the email receipt%4$s to upgrade. It only takes a few minutes to unlock the most powerful, yet easy to use analytics tracking system for WordPress.', 'exactmetrics-premium' ), '<a href="' . exactmetrics_get_upgrade_link( 'reports-page', $this->name . '-report-upsell-license-link' ) . '"><strong>', '</strong></a>', '<a style="text-decoration:underline !important" href="' . exactmetrics_get_url( 'reports-page', $this->name . '-report-go-lite-pro-link', 'https://www.exactmetrics.com/docs/go-lite-pro/' ) . '">', '</a>' ); // phpcs:ignore
										?>
									</p>
									&nbsp;
									<p><?php esc_html_e( 'If you have any questions, don\'t hesitate to reach out. We\'re here to help.', 'exactmetrics-premium' ); ?></p>
							...
							<div class="exactmetrics-upsell-card-action">
								<?php if ( exactmetrics_is_pro_version() ) { ?>
									<a href="<?php echo exactmetrics_get_upgrade_link( 'reports-page', $this->name . '-report-upsell-license-link' ); // phpcs:ignore -- Escaped in the function ?>"
									   class="exactmetrics-upsell-card-button"><?php esc_html_e( 'Upgrade Now', 'exactmetrics-premium' ); ?></a>
								<?php } else { ?>
									<a href="<?php echo exactmetrics_get_url( 'reports-page', $this->name . '-report-upsell-license-link', 'https://www.exactmetrics.com/docs/upgrade-exactmetrics-license/' ); // phpcs:ignore -- Escaped in the function ?>"
									   class="exactmetrics-upsell-card-button"><?php esc_html_e( 'Get ExactMetrics Pro', 'exactmetrics-premium' ); ?></a>
								<?php } ?>
							</div>
						</div>
					</div>
	...
	public function get_addons_page_link() {
		if ( exactmetrics_can_install_plugins() ) {
			...
			$addons_link = esc_html__( 'Please ask your webmaster to enable this addon.', 'exactmetrics-premium' );
		}
