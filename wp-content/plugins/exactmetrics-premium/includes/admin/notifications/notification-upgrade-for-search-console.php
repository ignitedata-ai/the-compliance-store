...
final class ExactMetrics_Notification_Upgrade_For_Search_Console extends ExactMetrics_Notification_Event {
	...
	public function prepare_notification_data( $notification ) {
		$notification['title'] = __( 'See Top Performing Keywords', 'exactmetrics-premium' );
		/* translators: Placeholders add a link to the upgrade url. */
		$notification['content'] = sprintf( __( '%1$sUpgrade to ExactMetrics Pro%2$s to see which keywords are driving traffic to your website so you can focus on what\'s working.', 'exactmetrics-premium' ), '<a href="' . $this->get_upgrade_url() . '" target="_blank">', '</a>' );
		$notification['btns']    = array(
			"get_exactmetrics_pro" => array(
				'url'         => $this->get_upgrade_url(),
				'text'        => __( 'Upgrade Now', 'exactmetrics-premium' ),
				'is_external' => true,
			),
		);
