...
function exactmetrics_modify_wordpress_autoupdater_setting( $html, $plugin_file, $plugin_data ) {
	...
	$is_main_free   = isset( $plugin_data['slug'] ) && 'exactmetrics-premium' === $plugin_data['slug'];
	...
	if ( $is_addon && ! $is_pro ) {
		$html = sprintf(
			...
			__( 'Enable the ExactMetrics PRO plugin to manage auto-updates', 'exactmetrics-premium' )
		);
...
function exactmetrics_automatic_updates( $update, $item ) {
	...
	$is_free = isset( $item['slug'] ) && 'exactmetrics-premium' === $item['slug'];
