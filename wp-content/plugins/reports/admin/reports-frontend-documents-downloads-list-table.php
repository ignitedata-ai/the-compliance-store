...
class Frontend_Documents_Downloads_List_Table extends WP_List_Table {
    ...
    function __construct() {
        ...
        parent::__construct(array(
            'singular' => __('Download', 'reports'), //singular name of the listed records
            'plural' => __('Downloads', 'reports'), //plural name of the listed records
    ...
    function column_title($item) {
        $delete_log_nonce = wp_create_nonce('downloads_delete_log_entry');
        //Build row actions
        $actions = array(
            'edit' => sprintf('<a href="' . admin_url('post.php?post=' . $item['ID'] . '&action=edit') . '">' . __('Edit', 'reports') . '</a>'),
            'delete' => sprintf('<a href="?page=frontend-document-downloads-report&action=%s&download=%s&row_id=%s&_wpnonce=%s" onclick="return confirm(\'Are you sure you want to delete this entry?\')">' . __('Delete', 'reports') . '</a>', 'delete', $item['ID'], $item['row_id'], $delete_log_nonce),
            'view-counts' => sprintf('<a href="' . admin_url('admin.php?page=frontend-document-downloads-report&action=specific-item-report&document_id=' . $item['ID'] ). '">' . __('View Counts', 'reports') . '</a>'),
        );
    ...
    function get_columns() {
        ...
        if(isset($_REQUEST['document_id']) && !empty($_REQUEST['document_id'])){
            ...
            $columns['counts'] = __('Number of Downloads', 'reports');
        }
    ...
    function process_bulk_action() {
        ...
        if ('delete2' === $this->current_action()) {
            ...
            foreach ($_POST['download'] as $item) {
                ...
                global $wpdb;
                $del_row = $wpdb->query('DELETE FROM ' . $wpdb->prefix . 'reports_downloads WHERE id = "' . $row_id . '"');
            }
            if ($del_row) {
        ...
        if ('delete' === $this->current_action()) {
            ...
            $action = 'downloads_delete_log_entry';
            if (!wp_verify_nonce($nonce, $action)){
                wp_die(__('Nope! Security check failed!', 'reports'));
            }
    ...
    function prepare_items() {
        ...
        if(isset($_REQUEST['downloads_filters_action'])) {
        ...
        if(isset($_REQUEST['document_id']) && !empty($_REQUEST['document_id'])){
            ...
            $dl_id = sanitize_text_field($_REQUEST['document_id']);
            $query = "SELECT * FROM $table_name WHERE $post_id = $dl_id ORDER BY $orderby_column $sort_order";
            $count_query = "SELECT COUNT(*) FROM ( SELECT $post_id FROM $table_name WHERE $post_id = $dl_id) AS count_query";
        ...
        if(isset($_REQUEST['document_id']) && !empty($_REQUEST['document_id'])) {
            ...
            foreach ($data_results as $data_result) {
                ...
                $countdownloads = "SELECT COUNT(*) FROM $table_name WHERE post_id = $data_result->post_id";
                $total_downloads = $wpdb->get_var($countdownloads); //for pagination requirement
                $counts = '<a href="' . admin_url('admin.php?page=frontend-document-downloads-report&action=specific-item-report&document_id=' . $data_result->post_id ). '">' . $total_downloads . '</a>';
            ...
            foreach ($data_results as $data_result) {
                ...
                $countdownloads = "SELECT COUNT(*) FROM $table_name WHERE post_id = $data_result->post_id";
                $total_downloads = $wpdb->get_var($countdownloads);
                $counts = '<a href="' . admin_url('admin.php?page=frontend-document-downloads-report&action=specific-item-report&document_id=' . $data_result->post_id ). '">' . $total_downloads . '</a>';
    ...
    function extra_tablenav( $which ) {
        ...
        if( $which == 'top' ){
            ...
            if($_REQUEST['page'] == 'frontend-document-downloads-report' && empty($_REQUEST['document_id'])) {
                ...
                submit_button( __( 'Filter' ), '', 'downloads_filters_action', false );
