    ...
    class PT_Export_Users
    {
        ...
        public function pt_query_call_for_all_export()
        {
            ...
            foreach ($all_users as $exported_user)
            {
                ...
                $business_name  = ( is_object($business_term) && isset($business_term->name) ) ? $business_term->name : '';
                $corporate_name = ( is_object($corporate_term) && isset($corporate_term->name) ) ? $corporate_term->name : '';
                $admin_name     = ( is_object($admin_term) && isset($admin_term->name) ) ? $admin_term->name : '';
                ...
                $create_date_raw = get_user_meta($exported_user, 'create_date', true);
                $create_date_formatted = '';
                if (!empty($create_date_raw)) {
                    if (is_numeric($create_date_raw)) {
                        // Treat it as Unix timestamp
                        $create_date_formatted = date('m/d/Y', (int)$create_date_raw);
                    } else {
                        $timestamp = strtotime($create_date_raw);
                        if ($timestamp !== false) {
                            $create_date_formatted = date('m/d/Y', $timestamp);
                        }
                    }
                }

                $start_date_raw = get_user_meta($exported_user, 'start_date', true);
                $start_date_formatted = '';
                if (!empty($start_date_raw)) {
                    if (is_numeric($start_date_raw)) {
                        // Treat it as Unix timestamp
                        $start_date_formatted = date('m/d/Y', (int)$start_date_raw);
                    } else {
                        $timestamp = strtotime($start_date_raw);
                        if ($timestamp !== false) {
                            $start_date_formatted = date('m/d/Y', $timestamp);
                        }
                    }
                }
            ...
            $html = "<div class='ftf-ajax-message' id='ftf-export-message'>
                        <p class='ftf-action'>
                            <a href='$file_url' id='ftf-communication-dos'></a>
                            <b>All users exported successfully.</b>
                        </p>
                    </div>

            <script type='text/javascript'>
                // Trigger the download after 1 second
                setTimeout(function() {
                    document.getElementById('ftf-communication-dos').click();
                }, 1000); 

                // Remove the message after 50 seconds
                setTimeout(function() {
                    var msg = document.getElementById('ftf-export-message');
                    if (msg) {
                        msg.remove();
                    }
                }, 10000);
                        ...
                        <div class="postbox">
                            ...
                            <div class="inside">
                                <form id="user_export_choose_date" method="post">
                                    <div>
                                        <h4 style="margin-bottom: 0;">Filter by Date Range (mm-dd-yyyy)</h4>
...
?>
            ...
            $selected = isset($section) ? selected($section, $value, false) : '';
...
?>
            ...
            if (isset($_POST['export-by-date']))
            {
                ...
                if (is_array($new_s_date) && count($new_s_date) >= 3) {                
                ...
                if (is_array($newdate) && count($newdate) >= 3) {  
                ...
                else
                {
                    ...
                    //                             LEFT JOIN $user_meta_table  um1 ON u.ID = um1.user_id
                    //                             GROUP BY u.ID");
                    echo"<p style='color:red; margin-top:-20px;'>Please select a company name or date range to export the list of the users.</p>";
                    exit;
                ...
                foreach ($res as $res_user)
                {
                    ...
                    $business_term_name = $corporate_term_name = $admin_term_name = "";

                    $res_user_obj = get_user_by('id', $res_user->ID);

                    $user_business_id = get_the_author_meta('business_companies', $res_user->ID);
                    if (!empty($user_business_id)) {
                        $business_term = get_term($user_business_id, 'company');
                        if (!is_wp_error($business_term) && isset($business_term->name)) {
                            $business_term_name = $business_term->name;
                        }
                    }

                    $user_corporate_id = get_the_author_meta('corporate_companies', $res_user->ID);
                    if (!empty($user_corporate_id)) {
                        $corporate_term = get_term($user_corporate_id, 'company');
                        if (!is_wp_error($corporate_term) && isset($corporate_term->name)) {
                            $corporate_term_name = $corporate_term->name;
                        }
                    }
                    ...
                    if (!empty($user_admin_id)) {
                        $admin_term = get_term($user_admin_id, 'company');
                        if (!is_wp_error($admin_term) && isset($admin_term->name)) {
                            $admin_term_name = $admin_term->name;
                        }
                    }
...
//test date 
                    $create_date_raw = get_user_meta($res_user->ID, 'create_date', true);
                    $create_date_formatted = '';
                    if (!empty($create_date_raw)) {
                        if (is_numeric($create_date_raw)) {
                            // Treat it as Unix timestamp
                    ...
                    $start_date_raw = get_user_meta($res_user->ID, 'start_date', true);
                    $start_date_formatted = '';
                    if (!empty($start_date_raw)) {
                        if (is_numeric($start_date_raw)) {
                            // Treat it as Unix timestamp
                    ...
                    $fields = array(
                        ...
                        $business_term_name,  // Use the safe variable here
                        $corporate_term_name, // Use the safe variable here
                        $admin_term_name,
