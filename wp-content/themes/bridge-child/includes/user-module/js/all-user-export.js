(function ($) {
  ...
  $("body").on("submit", "#na-pt-export-all", function (e) {
    e.preventDefault();
        // Disable the <input type="submit"> button
    var $submitButton = $(this).find('input[type="submit"][name="reports_export_users"]');
    $submitButton.prop("disabled", true).val("Exporting...");
  ...
  function query_call(data) {
    if (data > 0) {
      ...
      var $submitButton = $('#na-pt-export-all').find('input[type="submit"][name="reports_export_users"]');
      $submitButton.prop("disabled", false).val("Export All Users to CSV File");
    }
  }
})(jQuery);
