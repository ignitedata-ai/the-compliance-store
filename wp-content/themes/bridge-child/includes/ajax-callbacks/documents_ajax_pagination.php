...
function documents_ajax_pagination() {
    ...
    $taxonomy_term = json_decode(isset($_REQUEST['termid']) ? $_REQUEST['termid'] : '');
    ...
    $page = json_decode(isset($_REQUEST['page']) ? $_REQUEST['page'] : '');
    $offset = $page - 1;
    $start = (int) $offset * 25;
...
// The Query
    if ($pagnavposts->have_posts()) {
        while ($pagnavposts->have_posts()) {
            $pagnavposts->the_post();
            ...
            if ($cptName == 'documents') {
                ...
                $doc_content = '';
                if (!empty($document)) {
    //$doc_content = do_shortcode('[wonderplugin_pdf src="'.$document.'" width="100%" height="800px" style="border:0;"]');
...
add_action('wp_ajax_documents_ajax_pagination', 'documents_ajax_pagination');
